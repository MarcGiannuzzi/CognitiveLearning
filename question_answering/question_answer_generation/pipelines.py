...
class QGPipeline:
    """
        Description : Classe permettant de traiter la tâche de génération de questions.
    """
    ...
    def __init__(
        ...
        """
            Description : Fonction permettant d'initialiser un objet de la classe.
            Arguments : 
                - model : Le modèle permettant à la tâche d'être accomplie.
                - tokenizer : Le tokenizer permettant de tokeniser l'input en vue d'être utilisé par le modèle précédemment défini.
                ...
                - qg_format : Format des données utilisées par le modèle.
    ...
    def __call__(self, inputs: str):
        """
            Description : Fonction appelée lors de l'utilisation du modèle.
            Arguments : 
                - inputs : Inputs.
        """
...
class MultiTaskQAQGPipeline(QGPipeline):
    """
        Description : Classe permettant de traiter la tâche de génération de questions et de génération de réponses simultanément.
                      La classe hérite de QGPipeline, qui sert à générer des questions.
    """
...
class E2EQGPipeline:
    """
        Description : Classe permettant de traiter la tâche de génération de question avec une méthode End to End.
    """
    ...
    def _tokenize(
        ...
        """
            Description : Fonction permettant de tokeniser l'input en vue d'être utilisé par le modèle.
            Arguments : 
                - task : String représentant la tâche à accomplir.
                - inputs : Inputs.
                - padding : _x_
                - truncation : Format des données utilisées par le modèle.
                - add_special_tokens : Ajout ou non de tokens spéciaux.
...
def pipeline(
    ...
    **kwargs,
    ...
    """
        Description : Classe permettant de traiter la tâche de génération de question et de génération de réponses simultanément.
                    La classe hérite de QGPipeline, qui sert à générer des questions.
        Arguments : 
            - lang : Langue dont on veut la pipeline.
            - task : String représentant la tâche à accomplir.
            - model : Le modèle permettant à la tâche d'être accomplie.
            - tokenizer : Le tokenizer permettant de tokeniser l'input en vue d'être utilisé par le modèle précédemment défini.
            - qg_format : Format des données utilisées par le modèle.
    ...
    targeted_task = None
    if lang == 'french':
        if task not in SUPPORTED_TASKS_FR:
            raise KeyError("Unknown task {}, available tasks are {}".format(
                task, list(SUPPORTED_TASKS_FR.keys())))
        else:
            targeted_task = SUPPORTED_TASKS_FR[task]
    else:
        if task not in SUPPORTED_TASKS_EN:
            raise KeyError("Unknown task {}, available tasks are {}".format(
                task, list(SUPPORTED_TASKS_EN.keys())))
        else:
            targeted_task = SUPPORTED_TASKS_EN[task]
